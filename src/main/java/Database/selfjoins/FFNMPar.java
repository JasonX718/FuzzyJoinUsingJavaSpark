 ...
 *
 */
public class FFNMPar implements Serializable {
...
//	public static int index = 0;
	...
	public void closeSparkContext() {
		FFNMPar.javasparkcontext.close();
	}

	public FFNMPar() throws ClassNotFoundException {
	...
	public void run(String input0, String output, int eps) throws IOException, InterruptedException {
		...
		BitSet input = new BitSet(CONST_FILTERS.vectorsize);
		for(String key: keys) {
			input.set(Integer.valueOf(key));
		}
		...
		final Broadcast<BallListInt> bcballs = javasparkcontext.broadcast(ff);
		...
		JavaPairRDD<String, Tuple2<String,String>> pair0 = pairRDD_0.flatMapToPair(new PairFlatMapFunction<Tuple2<String,String>, String, Tuple2<String,String>>() {
			private static final long serialVersionUID = 1L;
			...
			public Iterator<Tuple2<String, Tuple2<String,String>>> call(Tuple2<String,String> t) throws Exception {
				...
				//while(t.hasNext()) {
					//Tuple2<String,String> tuple = t.next();
					//int p = Math.abs(CONST_FILTERS.hashFunction.hash(tuple._1.getBytes())) % CONST_FILTERS.vectorsize;
					if(t._1.length()==CONST.KEY_MINI_LENGTH) {
						int p=Integer.valueOf(t._1);
						ArrayList<Integer> ball = ff.getBall(p); 
							if(ball!=null && ball.size()>0) {
								for(int i:ball) {
									//if(i < p)
										out.add(new Tuple2<String, Tuple2<String,String>>(String.valueOf(i), new Tuple2<String,String>(String.valueOf(p),t._2)));
								}
							}
							out.add(new Tuple2<String, Tuple2<String,String>>(String.valueOf(p), new Tuple2<String,String>("-1",t._2)));
					}
				//}
				return out.iterator();
			}
...
//				} 
	...
	public static void main(String[] userargs) {
		try {
			...
			FFNMPar sparkProgram = new FFNMPar();
