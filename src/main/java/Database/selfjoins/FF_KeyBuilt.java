 ...
 *
 */
public class FF_KeyBuilt implements Serializable {
...
//	public static int index = 0;
	...
	public void closeSparkContext() {
		FF_KeyBuilt.javasparkcontext.close();
	}

	public FF_KeyBuilt() throws ClassNotFoundException {
		sparkconf = new SparkConf().setAppName("FF fuzzy self join key built");
	...
	public void run(String input0, String output, int eps) throws IOException, InterruptedException {
		...
		for(String keyarr: keychain) {
			if(keyarr.length()>0) {
				...
				for(String k:arr)
					inputset.set(Integer.valueOf(k));
			}
		}
...
//				} 
	...
	public static void main(String[] userargs) {
		try {
			...
			FF_KeyBuilt sparkProgram = new FF_KeyBuilt();
