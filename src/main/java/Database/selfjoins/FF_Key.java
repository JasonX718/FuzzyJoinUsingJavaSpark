 ...
 *
 */
public class FF_Key implements Serializable {
...
//	public static int index = 0;
	...
	public void closeSparkContext() {
		FF_Key.javasparkcontext.close();
	}

	public FF_Key() throws ClassNotFoundException {
		sparkconf = new SparkConf().setAppName("FF fuzzy self join key");
	...
	public void run(String input0, String output, int eps) throws IOException, InterruptedException {
		...
		JavaRDD<String> keys = pairRDD_0.keys().mapPartitions(new FlatMapFunction<Iterator<String>, String>() {
			private static final long serialVersionUID = 1L;

			@Override
			public Iterator<String> call(Iterator<String> t) throws Exception {
				ArrayList<String> out = new ArrayList<>();
				BitSet tmpset = new BitSet(CONST_FILTERS.vectorsize);
				String tmpstr;
				while(t.hasNext()) {
					tmpstr = t.next();
					if(tmpstr!=null && tmpstr.length()==CONST.KEY_MINI_LENGTH) {
						tmpset.set(Integer.valueOf(tmpstr));
					}
				}
		...
		List<String> keychain = keys.collect();
		BitSet input = new BitSet(CONST_FILTERS.vectorsize);
		for(String keyarr: keychain) {
			if(keyarr.length()>0) {
				String[] arr = keyarr.split(", ");
				for(String k:arr)
					input.set(Integer.valueOf(k));
			}
		}
...
//				} 
	...
	public static void main(String[] userargs) {
		try {
			...
			FF_Key sparkProgram = new FF_Key();
