...
public class StructSimil implements Serializable {
		...
		return dataset.mapToPair(new PairFunction<String,String,String>() {
			...
			public Tuple2<String, String> call(String s) throws Exception {
				if (pumakey.isKeyPositionPossible(s, key_position)) {
					...
					// Pre-process the input string to ensure it is in a valid JSON format
					...
					List<Tuple2<Integer,Integer>> hashBranches = JsonParseHelper.getBranchsJsonHash(rootNode, mode);
	
					int [][] adjacencyMatrix = SimilarityHelper.getAdjacencyMatrixHash(hashBranches);
	
					String strAdjacency = SimilarityHelper.matrixToString(adjacencyMatrix);
	
					return new Tuple2<>(strAdjacency, s);
				} else {
					throw new Exception("The key position is not possible");
				}
			}
	
			// Utility method to preprocess the input into a valid JSON string
	...
	// 			// If the input contains an underscore, split it into key-value pairs
	// 			if (input.contains("_")) {
	// 				String[] parts = input.split("_", 2); // Split into key and value
	
	// 				// Check if the key is numeric, which might indicate a different structure
	// 				String key = parts[0];
	// 				String value = parts[1];
	
	// 				// If the key is a number, treat it as an index
	// 				if (key.matches("\\d+")) {
	// 					return String.format("{\"index\": \"%s\", \"value\": \"%s\"}", key, value);
	// 				} else {
	// 					return String.format("{\"%s\": \"%s\"}", key, value);
	// 				}
	// 			}
	
	// 			// If the input does not match any specific format, wrap it in a generic JSON object
	// 			return String.format("{\"value\": \"%s\"}", input);
	// 		}
	// 	});
	// }
	 ...
	 */
	public void fuzzyJoin(JavaPairRDD<String, String> pairRDD_1, String outputPath, int eps, GraphCreationMode mode) {
		
		JavaPairRDD<String, Tuple2<String, String>> couplesRDD = pairRDD_1.join(pairRDD_1);
		...
		/*for(Tuple2<String, String> record : pairCoupleDistinctRDD.collect()){
			...
			Tuple2<Double,Integer> resultBestHammingDistance = bestHammingDistance(json1, json2, mode);
	 ...
	 */
	public void run(String inputPath, String outputPath, int key_position, int eps, GraphCreationMode mode) {
		...
		JavaPairRDD<String, String> pairRDD_dataset_1 = createJavaPairRDD(dataset, key_position, mode);

		JavaRDD<String> merged_pairRDD_dataset_1 = pairRDD_dataset_1.map(pair -> pair._1 + "|" + pair._2);

		int maxLength = pumakey.getMaxKeyLength(key_position, merged_pairRDD_dataset_1);

		ArrayList<String> vocabulary = new ArrayList<String>() {{add("0");add("1");}};
		JavaPairRDD<String, String> BHpairRDD_dataset_1 = BH1.createBHJavaPairRDD(merged_pairRDD_dataset_1, vocabulary, key_position, eps, maxLength, outputPath);

		createResultFile(outputPath+"_AdjacencyMatrix", BHpairRDD_dataset_1.collect());

		fuzzyJoin(BHpairRDD_dataset_1, outputPath, eps, mode);
