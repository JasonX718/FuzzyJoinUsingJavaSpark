...
/**
 * Implements a <i>Difference Filter with Lossy Hash Table bucket=128 bits of md5 (32 characters)</i>, as defined by PHAN Thuong Cang
 */

public class DifferenceStringFilter implements Writable  {

	DynamicBloomFilter  dbf;
	LossyHashStringFilter lhf;


	public DifferenceStringFilter(){}
	 ...
	 * @param n0 the maximum number of keys to record in a standard Bloom filter row.
	 */
	public DifferenceStringFilter(int vectorSizeDBF, int nbHashDBF, int hashTypeDBF, int n0, int vectorSizeLHF, int nbHashLHF, int hashTypeLHF ) {
		dbf = new DynamicBloomFilter(vectorSizeDBF, nbHashDBF, hashTypeDBF, n0); // n0 the maximum number of keys
		lhf = new LossyHashStringFilter(vectorSizeLHF, nbHashLHF, hashTypeLHF);  // the maximum number of buckets, the number of Hash functions, the type of hash function
	}

	//Quyen's add function
		public DifferenceStringFilter(DifferenceStringFilter copy) {
			dbf = new DynamicBloomFilter(copy.getDBF());
			lhf = new LossyHashStringFilter(copy.getLHF());
		}
		...
		public LossyHashStringFilter getLHF() {
			return lhf;
		}
	...
	public void and(DifferenceStringFilter filter) {
		if (filter == null){
	...
	public void or(DifferenceStringFilter filter) {
		if (filter == null){
			throw new IllegalArgumentException("filters cannot be or-ed");
		}
...
//	@SuppressWarnings("deprecation")
	public static DifferenceStringFilter generateHdfsUnionFilter(String FILTER_INPUT_HDFS_DIRECTORY, String UNIONFILTER_OUTPUT_HDFS_FILE, int iteration, Configuration conf) throws IOException{
		DifferenceStringFilter  unionDF = new DifferenceStringFilter();
		DifferenceStringFilter df = new DifferenceStringFilter();
...
//				System.out.println("\n*********Dang doc bo loc thu" + i + " : " + status[i].getPath().toString());
				...
				if (unionDF == null) {
					...
					unionDF = new DifferenceStringFilter(df);
					//unionDF = df;
				}
				else {*/
					unionDF.or(df);
...
//					System.out.println("\n++++++++Da viet bo loc Union DF len HDFS ");
	...
	public static DifferenceStringFilter generateHdfsUnionFilter(String previous_DF, String FILTER_INPUT_HDFS_DIRECTORY, String UNIONFILTER_OUTPUT_HDFS_FILE, int iteration, Configuration conf) throws IOException{
		DifferenceStringFilter  unionDF = new DifferenceStringFilter();
		DifferenceStringFilter df = new DifferenceStringFilter();
...
//					System.out.println("\n++++++++Da viet bo loc Union DF len HDFS ");
	...
	public static DifferenceStringFilter generateHdfsUnionFilter(String previous_DF, String FILTER_INPUT_HDFS_DIRECTORY, String UNIONFILTER_OUTPUT_HDFS_FILE, int iteration, Configuration conf, StringBuilder message) throws IOException{
		DifferenceStringFilter  unionDF = new DifferenceStringFilter();
		DifferenceStringFilter df = new DifferenceStringFilter();
...
//				unionDF = df;
	...
	public static void main(String[] args) {
		...
		DifferenceStringFilter df = new DifferenceStringFilter(30, 3, Hash.MURMUR_HASH, 10 ,    10, 1, Hash.MURMUR_HASH);
		...
		String URI = new String("file:");
		String compressionFilePathName = new String("/home/ptcang/Quyen/thuat/test.bloom.gz");
		...
		DifferenceStringFilter  df2 = new DifferenceStringFilter() ; // No parameters here, parameters will be load from file
		try {
