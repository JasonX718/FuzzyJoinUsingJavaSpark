...
public class BH1MPar implements Serializable {
	...
	public void closeSparkContext() {
		BH1MPar.javasparkcontext.close();
	}
	//Accumulator
	//persist
	//sequenceFile[K, V]
	//rdd.collect().foreach(println)-> rdd.take(100).foreach(println)
	public BH1MPar() throws ClassNotFoundException {
		sparkconf = new SparkConf().setAppName("BH1 fuzzy 2way join");
		javasparkcontext = new JavaSparkContext(sparkconf);
		hdfsPath = "hdfs://" + CONST.NAMENODE + ":9000";
	}
	
	public static JavaPairRDD<String, String> createJavaPairRDD(JavaRDD<String> lines, int keyCol, int eps)
	...
	public static JavaPairRDD<String, String> createBHJavaPairRDD(JavaRDD<String> lines, int keyCol, int eps)
			throws NullPointerException {
		return lines.mapPartitionsToPair(new PairFlatMapFunction<Iterator<String>, String, String>() {
			private static final long serialVersionUID = 1L;
				...
				while(t.hasNext()) {
					...
					if (tmpstr!=null && tmpstr.length()==(CONST.KEY_MINI_LENGTH)) {
								try {
									...
									BallOfRadius.generateBHBall2way(tmpch, tmpch, tmpch.length-1, eps, out,tuple,tmpstr);
									out.add(new Tuple2<String, String>(tmpstr, tuple));
								} catch(Exception e) {
...
//		}
	...
	public void run(String input0, String input1, String output, int eps) {
		...
		JavaRDD<String> lines1 = javasparkcontext.textFile(hdfsPath + input1);//, CONST.NUM_PARTITION); // OK!
		
		JavaPairRDD<String, String> pairRDD_0 = createJavaPairRDD(lines0, CONST.FIRST_COL,eps);//.partitionBy(CONST.partition); // OK!
		JavaPairRDD<String, String> pairRDD_1 = createBHJavaPairRDD(lines1, CONST.SECOND_COL,eps);//.partitionBy(CONST.partition); // OK!
...
//		System.out.println(pairRDD_0.groupByKey().filter(t->t._1.length()==CONST.LENGTH).count());
	...
	public static void main(String[] userargs) {
		try {
			...
			BH1MPar sparkProgram = new BH1MPar();
