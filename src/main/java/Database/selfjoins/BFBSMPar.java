					...
					if (tmpstr != null && tmpstr.length() ==CONST.KEY_MINI_LENGTH) {
						...
						//if (eps==0) {
							//if(CONST_FILTERS.membershipTestBF(set, tmpstr)) //selfjoin k loc duoc
								tmpout.add(new Tuple2<String, String>(tmpsplits[0], tuple._2));
						//} 
						//else {
						if (eps>0) {
							...
							//if(CONST_FILTERS.membershipTestBF(set, tmpstr)) {
								BallOfRadius.generateBallSplitBFtest(tmpch, tmpch, tmpch.length - 1, eps, tmpout, set,tuple._2,tmpsplits,indexsplits,eps);
							//}
						//}
						}
					}
