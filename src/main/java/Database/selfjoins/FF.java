...
//	public static int index = 0;
	...
	public void closeSparkContext() {
		FF.javasparkcontext.close();
	}

	public FF() throws ClassNotFoundException {
		sparkconf = new SparkConf().setAppName("FF fuzzy self join");
	...
	public void run(String input0, String output, int eps) throws IOException, InterruptedException {
		...
		//JavaPairRDD<String,String> pairRDD_0 = createJavaRDD2(lines0, CONST.FIRST_COL);//.repartition(CONST.NUM_PARTITION);//.distinct();
		...
		// Dataset will persist in memory
		pairRDD_0.persist(CONST.STORAGE_LEVEL);

		// BF filter
		List<String> keys = pairRDD_0.keys().collect(); // collect all keys
		BitSet input = new BitSet(CONST_FILTERS.vectorsize); // create a bitset to store all keys (all 0)
		for(String key: keys) {
			input.set(Integer.valueOf(key)); // set 1 for each key in the bitset
		}

		BallListInt ff = new BallListInt(input, eps); // Iniitialize the ball list
		long te = System.currentTimeMillis(); 
		System.out.println("preproceessing running time " + (te - ts) / 1000 + " s");
		ts = System.currentTimeMillis();
		
		final Broadcast<BallListInt> bcballs = javasparkcontext.broadcast(ff); // broadcast the ball list to all nodes

		JavaPairRDD<String, Tuple2<String,String>> pair0 = pairRDD_0.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Tuple2<String,String>>, String, Tuple2<String,String>>() { // map each key to a ball
			private static final long serialVersionUID = 1L;

			@Override
			public Iterator<Tuple2<String, Tuple2<String,String>>> call(Iterator<Tuple2<String,String>> t) throws Exception { // t is an iterator of key value pairs
				ArrayList<Tuple2<String, Tuple2<String,String>>> out = new ArrayList<>(); // output
				BallListInt ff = bcballs.value(); // get the ball list
				while(t.hasNext()) {
...
//		pair0.mapToPair(t->new Tuple2<String,Tuple2<String,String>>(t._1, new Tuple2<String,String>(t._2._1,pumakey.getRKey(t._2._2, CONST.FIRST_COL)))).groupByKey().saveAsTextFile(hdfsPath + output);
