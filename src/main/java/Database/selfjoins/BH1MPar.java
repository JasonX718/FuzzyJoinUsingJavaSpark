...
public class BH1MPar implements Serializable {
	...
	public static JavaPairRDD<String, Tuple2<String,String>> createBHJavaPairRDD(JavaRDD<String> lines, int keyCol, int eps)
		...
		return lines.mapPartitionsToPair(new PairFlatMapFunction<Iterator<String>, String, Tuple2<String,String>>() {
			private static final long serialVersionUID = 1L;
			
			@Override
			public Iterator<Tuple2<String, Tuple2<String,String>>> call(Iterator<String> t) throws Exception {
...
//						if(!tmpstr.endsWith("R")) {
							try {
								...
								boolean b=false;
								b=BallOfRadius.generateBHBallTuple(tmpch, tmpch, tmpch.length-1, eps, out,tuple,b,tmpstr);// Corriger 0
								//if (b) 
									out.add(new Tuple2<String, Tuple2<String,String>>(tmpstr, new Tuple2<String,String>("-1",tuple)));
						...
						if(skey.length()==CONST.KEY_MINI_LENGTH) {
							if(s2list.size()>0) {
								...
								for (String b : s2list) {
									...
									if(bkey.length()==CONST.KEY_MINI_LENGTH)
